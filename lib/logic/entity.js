Bloob.Entity = function() {
	this.isSticky = false;
};

Bloob.Entity.prototype.update = function(timePassed) {};

Bloob.Entity.prototype.__update__ = function(timePassed) {
	if(this.isSticky) {
		this.removeStickyJoints();
		this.attachStickyJoints();
	}
	return this.update(timePassed);
};

Bloob.Entity.prototype.setBody = function(body) {
	this.body = body;
	body.entity = this;
...
Bloob.Entity.prototype.getBody = function() { return this.body; };

/*
 *  Stickyness:
 */
Bloob.Entity.prototype.makeSticky = function() {
	this.isSticky = true;
	this.stickies = [];

	this.maxStickyDistance = 1.3;
	gui.add(this, 'maxStickyDistance').name('StickyBreak').min(0).max(10).listen().step(0.1);

	this.createStickyJoints();
	this.pointMassIsConnected = Bloob.Utils.fillArray(false, this.body.mPointCount);
	
	return this;
};

Bloob.Entity.prototype.createStickyJoints = function() {
	var body = this.body;
	...
	for (var i = 0; i < body.mPointCount; i++)
	{
		...
		var temp = jointInfo.temp = Bloob.MapBuilder.buildParticleAt(
			world,
			this.body.pointMasses[i].Position,
			0.0,
			Vector2.One.copy()
		);
		jointInfo.jointToTemp = new DistanceJoint(
			world,
			this.body, i,
			temp, 0,
			0, 
		...
		jointInfo.jointTempToA = new DistanceJoint(
			world,
			temp, 0,
		...
		jointInfo.jointTempToB = new DistanceJoint(
			world,
			temp, 0,
			temp, 0, // to replace: second pm
			0, // to replace: dist
...
Bloob.Entity.prototype.attachStickyJoints = function() {
	/*
...
Bloob.Entity.prototype.removeStickyJoints = function() {
 ...
 *  Handle Contacts:
 */
Bloob.Entity.prototype.onContact = function(otherBody, contact) {

};

Bloob.Entity.prototype.onStartContact = function(otherBody, contact) {
	//Scarlet.log("Start: ", this.body.aName);
};

Bloob.Entity.prototype.onEndContact = function(otherBody) {
	//Scarlet.log("End: ", this.body.aName);
};
