...
Bloob.BodyBluePrint.prototype.build = function() {
	var newBody = new this.store.targetClass(
		this.store.world,
		this.store.shape,
		this.store.pointMasses,
		this.store.translate,
		this.store.rotate,
		this.store.scale,
		this.store.isKinematic,
		this.store.edgeSpringK,
		this.store.edgeSpringDamp,
		this.store.shapeSpringK,
		this.store.shapeSpringDamp,
		this.store.gasPressure
	);

	for(var i = 0; i < this.store.internalSprings.length; i++)
		newBody.addInternalSpring.apply(newBody, this.store.internalSprings[i]);
