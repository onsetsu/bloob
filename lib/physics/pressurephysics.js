define(["physics/vector2"], function(Vector2) {
	var PressurePhysics = function(body, bodyDefinition) {
		...
		for(var i = 0; i < body._pointMassCount.length; i++) {
	...
	PressurePhysics.prototype.accumulateInternalForces = function(body) {
		...
		for (var i = 0; i < body._pointMassCount; i++)
		{
			...
			var prev = (i > 0) ? i-1 : body._pointMassCount-1;
			var next = (i < body._pointMassCount - 1) ? i + 1 : 0;
		...
		for (var i = 0; i < body._pointMassCount; i++)
		{
			...
			var j = (i < body._pointMassCount - 1) ? i + 1 : 0;
