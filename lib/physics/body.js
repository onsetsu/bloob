...
//		edgeD // float&
	...
	Body.prototype.addGlobalForce = function(pt, force) { //  const Vector2&,  const Vector2&
		...
		for (var i = 0; i < this.pointMasses.length; i++)
		{
			...
			var torque = VectorTools.rotateVector(toPt, -JellyPrerequisites.HALF_PI); // Vector2
	...
	Body.prototype.debugDrawAngle = function(debugDraw) {
		...
		var up = VectorTools.rotateVector(new Vector2(0, 1), this.mDerivedAngle);
		debugDraw.drawPolyline([
	...
	Body.prototype.debugDrawTypeAndId = function(debugDraw) {
		...
		debugDraw.context.textBaseline = 'top';

		debugDraw.drawTextWorld(
			this.type + ": " + this.id + this.useSprings + this.usePressure,
			new Vector2(
				this.mAABB.Min.x,
				this.mAABB.Max.y
			)
	...
	Body.prototype.isSpringBody = function() {
		return this.useSprings;
	};

	Body.prototype.isPressureBody = function() {
