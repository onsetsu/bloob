...
var Body = function(
	...
	if(!(bodyDefinition.getPointMasses() instanceof Array))
		bodyDefinition.setPointMasses(Utils.fillArray(bodyDefinition.getPointMasses(), bodyDefinition.getShape().getVertices().length));
...
Body.prototype.debugDraw = function(debugDraw) {
	...
	this.debugDrawTypeName(debugDraw);
...
Body.prototype.debugDrawAngle = function(debugDraw) {
	...
	var up = Jello.VectorTools.rotateVector(new Vector2(0, 1), this.mDerivedAngle);
	debugDraw.drawPolyline([
...
Body.prototype.debugDrawTypeName = function(debugDraw) {
	debugDraw.setOptions({
		"color": "lightgrey",
		"opacity": 0.8,
		"lineWidth": 1
	...
	debugDraw.drawTextWorld(
		this.type + ": " + (this.aName || this.id || "body"),
		new Vector2(
			this.mAABB.Min.x,
			this.mAABB.Max.y
		)
