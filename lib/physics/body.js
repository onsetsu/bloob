...
define(function(require) {
    var num = require('num/num'),
        ...
        constants = num.constants,
        Vector2 = num.Vector2,
        VectorTools = num.VectorTools,
        Bitmask = require('./bitmask'),
        CollisionType = require('./collisiontype'),
        utils = require('./utils'),
        PointMass = require('./pointmass'),
        AABB = require('./aabb'),
        ClosedShape = require('./closedshape'),
        Material = require('./materialmanager'),
        InternalSpring = require('./internalspring'),
        SpringPhysics = require('./springphysics'),
        PressurePhysics = require('./pressurephysics');
	...
	Body.prototype._setDefaultValues = function() {
		...
		this._lastAngle = 0.0;
		this._transform = AffineTransformation.Identity.copy();
		this._aabb = new AABB();
	...
	Body.prototype._setShape = function(shape, forceUpdate) {
		...
		if (this._baseShape.getVertices().length != this._pointMassCount || forceUpdate)
		{
            ...
            var i;
			for(i = 0; i < shape.getVertices().length; i++)
			...
			for(i = 0; i < this._baseShape.getVertices().length; i++)
	...
	Body.prototype.derivePositionAndAngle = function(elapsed) {
		...
		for(i = 0; i < this._pointMassCount; i++)
		{
		...
		for(i = 0; i < this._pointMassCount; i++) {
			...
			if (i === 0) {
				...
				if ((utils.absf(diff) > constants.PI) && (thisSign != originalSign)) {
					thisAngle = (thisSign == -1) ? (constants.PI + (constants.PI + thisAngle)) : ((constants.PI - thisAngle) - constants.PI);
				}
			}
		...
		if (utils.absf(angleChange) >= constants.PI)
		{
			if (angleChange < 0.0)
				angleChange = angleChange + constants.TWO_PI;
			else
				angleChange = angleChange - constants.TWO_PI;
		}
	...
	Body.prototype.updateEdgeInfo = function(forceUpdate) { // bool(have to be updated?)
		if(((!this._isStatic) && (!this._ignoreMe)) || (forceUpdate)) {
			for (var i = 0; i < this._boundaryCount; i++) {
				...
				this._edgeInfo[i].slope = (utils.absf(e.y) < 1.0e-08) ? 0.0 : (e.x / e.y);
			}
		}
	};
	...
	Body.prototype.getMass = function() {
		return this._pointMasses.reduce((accumulatedMass, pointMass) => accumulatedMass + pointMass.Mass, 0);
	};
	...
	Body.prototype.addGlobalForce = function(pt, force) { //  const Vector2&,  const Vector2&
		...
		for (var i = 0; i < this._pointMassCount; i++)
		{
			...
			var torque = toPt.rotate(-constants.HALF_PI); // Vector2
