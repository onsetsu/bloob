define([
	...
	Body.prototype.updateEdgeInfo = function(forceUpdate) { // bool(have to be updated?)
		if(((!this._isStatic) && (!this._ignoreMe)) || (forceUpdate)) {
			for (var i = 0; i < this._pointMassCount; i++) {
				var j = (i < (this._pointMassCount-1)) ? i + 1 : 0; // int
	...
	Body.prototype.updateAABB = function(elapsed, forceUpdate) { // float, bool 
		...
		if (((!this._isStatic) && (!this._ignoreMe)) || (forceUpdate)) {
			...
			//printf("Body: %d AABB: min[%f][%f] max[%f][%f]\n", this, _aabb.Min.X, _aabb.Min.Y, _aabb.Max.X, _aabb.Max.Y);
		};
	};
	...
	Body.prototype.contains = function(pt) { // Vector2
		...
		var c = this._pointMassCount; // int
		for(var i = 0; i < c; i++) {
...
//		edgeD // float&
	...
	//----------------------------------------------
		...
		var ptA = this._pointMasses[outParams.edgeNum].Position; // Vector2
		var ptB; // Vector2
		
		if (outParams.edgeNum < (this._pointMassCount - 1))
			ptB = this._pointMasses[outParams.edgeNum + 1].Position;
		else
			ptB = this._pointMasses[0].Position;
		
		var toP = outParams.pt.sub(ptA); // Vector2
		var E = this._edgeInfo[outParams.edgeNum].dir; // Vector2
		
		// get the length of the edge, and use that to normalize the vector.
		var edgeLength = this._edgeInfo[outParams.edgeNum].length; // float
	...
	Body.prototype.getIsKinematic = function() { return this._isKinematic; }; // returns bool
	Body.prototype.setIsKinematic = function(val) { this._isKinematic = val; }; // bool
	...
	Body.prototype.debugDrawPolygon = function(debugDraw) {
		debugDraw.drawPolyline(_.pluck(this._pointMasses, "Position"), "white", 1.0, 1);
	};
