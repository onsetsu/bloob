define([
	...
	Body.prototype._setDefaultValues = function() {
		...
		this.mCollisionType = CollisionType.getDefault();
	...
	Body.prototype._buildFromDefinition = function(bodyDefinition) {
		this.mWorld = bodyDefinition.getWorld();
		this.mDerivedPos = bodyDefinition.getPosition();
		this.mDerivedAngle = bodyDefinition.getAngleInRadians();
		this.mLastAngle = this.mDerivedAngle;
		this.mScale = bodyDefinition.getScale();
		this.mMaterial = bodyDefinition.getMaterial();
		this.mCollisionType = bodyDefinition.getCollisionType();
		this.mPointCount = 0;
		this.mInversePointCount = 0.0;

		// TODO: check for all zero masses -> static
		this.mIsStatic = false; // || bodyDefinition.pointMasses.all == 0.0;
		this.mKinematic = bodyDefinition.getIsKinematic();
		...
		this.setShape(bodyDefinition.getShape());

		// attach mass values to each pointMass in shape
		for (var i = 0; i < this.mPointCount; i++)
			this.pointMasses[i].Mass = bodyDefinition.getPointMasses()[i];
	...
	Body.prototype.getCollisionType = function() { return this.mCollisionType; };
	Body.prototype.setCollisionType = function(val) { this.mCollisionType = val; };
	...
	Body.prototype.updateEdgeInfo = function(forceUpdate) { // bool(have to be updated?)
		if(((!this.mIsStatic) && (!this.mIgnoreMe)) || (forceUpdate)) {
			for (var i = 0; i < this.mPointCount; i++) {
				...
				this.mEdgeInfo[i].slope = (JellyPrerequisites.absf(e.y) < 1.0e-08) ? 0.0 : (e.x / e.y);
			};
		};
	};
	...
	Body.prototype.updateAABB = function(elapsed, forceUpdate) { // float, bool 
		...
		if (((!this.mIsStatic) && (!this.mIgnoreMe)) || (forceUpdate)) {
			...
			for(var i = 0; i < this.pointMasses.length; i++) {
				...
				if (!this.mIsStatic) {
					p.weightedAddSelf(this.pointMasses[i].Velocity, elapsed);
					this.mAABB.expandToInclude(p);
				};
			};
...
//		edgeD // float&
	//}
	//----------------------------------------------
		outParams.hitPt = Vector2.Zero.copy();
		outParams.norm = Vector2.Zero.copy();
	...
	Body.prototype.debugDrawGlobalShape = function(debugDraw) {
		debugDraw.drawPolyline(this.mGlobalShape, "gray", 0.5, 1);
	};

	Body.prototype.debugDrawPolygon = function(debugDraw) {
		debugDraw.drawPolyline(_.pluck(this.pointMasses, "Position"), "white", 1.0, 1);
	};

	Body.prototype.debugDrawPointMasses = function(debugDraw) {
		for(var i = 0; i <  this.pointMasses.length; i++)
			debugDraw.drawRectangle(this.pointMasses[i].Position, 3, "white", 1.0);
	};

	Body.prototype.debugDrawMiddlePoint = function(debugDraw) {
		debugDraw.drawRectangle(this.mDerivedPos, 5, "lightgreen", 1.0);
	};

	Body.prototype.debugDrawVelocity = function(debugDraw) {
		debugDraw.drawLine(
			this.mDerivedPos,
			this.mDerivedPos.add(this.mDerivedVel),
			"violet",
			1.0,
			1
		);
	...
	Body.prototype.debugDrawAngle = function(debugDraw) {
		var up = Vector2.YAxis.rotate(this.mDerivedAngle);
		debugDraw.drawLine(
			this.mDerivedPos.add(up),
			this.mDerivedPos,
			"red",
			1.0,
			1
	...
	Body.prototype.debugDrawTypeAndId = function(debugDraw) {
		debugDraw.drawTextWorld(
			...
			new Vector2(this.mAABB.Min.x, this.mAABB.Max.y),
			"lightgrey",
			0.8,
			"top"
		);
	...
	Body.prototype.debugDrawTarget = function(debugDraw) {
		...
		var target = this.mWorld.getBodyById(this.target);
	...
	Body.prototype.isVisible = function(debugDraw) {
		return debugDraw.isVisible(this.mAABB);
	};
	...
	Body.prototype.toJson = function() {
		...
		for(var i = 0; i < this.pointMasses.length; i++)
			resultJson.pointMasses.push(this.pointMasses[i].Mass);
