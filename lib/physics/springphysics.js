define([
	...
	SpringPhysics.prototype._buildDefaultSprings = function() {
		for (var i = 0; i < this.body._boundaryCount; i++)
		{
			if (i < (this.body._boundaryCount - 1))
	...
	SpringPhysics.prototype.addInternalSpring = function(pointA, pointB, springK, damping ) { // int, int, float, float
		var dist = this.body._pointMasses[pointB].Position.distance(this.body._pointMasses[pointA].Position); // float 
	...
	SpringPhysics.prototype.setSpringConstants = function(springID, springK, springDamp) { // int, float, float
		...
		var index = this.body._boundaryCount + springID; // int
	...
	SpringPhysics.prototype.accumulateInternalForces = function() {	
		...
		for (var it = 0; it < this.mSprings.length; it++)
		{
			...
			var pmA = this.body._pointMasses[s.pointMassA]; // PointMass
			var pmB = this.body._pointMasses[s.pointMassB]; // PointMass
			
			if (false)//i < this.body._pointMassCount) or _boundaryCount
			{
				...
				force = VectorTools.calculateSpringForce(
					this.body._edgeInfo[i].dir.negative(), this.body._edgeInfo[i].length, pmA.Velocity, pmB.Velocity,
		...
		{
			this.body._baseShape.transformVertices(
				this.body._derivedPosition,
				this.body._derivedAngle,
				this.body._scale,
				this.body._globalShape
			);
			for (var i = 0; i < this.body._pointMassCount; i++)
			{
				var pmA = this.body._pointMasses[i]; // PointMass
				
				if (this.mShapeSpringK > 0)
				{
					if (!this.body._isKinematic)
					{
						force = VectorTools.calculateSpringForceVelAVelB(
							pmA.Position, pmA.Velocity,
							this.body._globalShape[i], pmA.Velocity,
					...
					else
					{
						force = VectorTools.calculateSpringForceVelAVelB(
							pmA.Position, pmA.Velocity,
							this.body._globalShape[i], Vector2.Zero.copy(),
							0.0, this.mShapeSpringK, this.mShapeSpringDamp
						);
					}
	...
	SpringPhysics.prototype.toJson = function(resultJson) {
		...
		for (var i = this.body._boundaryCount; i < this.mSprings.length; i++)
