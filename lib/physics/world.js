...
World.prototype.getBodyById = function(id) { // returns Body
	for(var i = 0; i < this.mBodies.length; i++)
...
World.prototype.getBodyByName = function(name) { // returns Body
	for(var i = 0; i < this.mBodies.length; i++)
		if(this.mBodies[i].aName == name) {
			return this.mBodies[i];
		};
	return undefined;
};
...
World.prototype._goNarrowCheck = function(bI, bJ) { // Body*, Body*
	...
	// TODO: use CollisionTypes instead of this.
	/*
...
World.prototype._handleCollisions = function() {
	...
	for (var i = 0; i < this.mCollisionList.length; i++)
	{
		...
		*/
		...
		//this.materialManager.getMaterialPair(info.bodyA.getMaterial(), info.bodyB.getMaterial()).Elasticity; // float
		...
		//this.materialManager.getMaterialPair(info.bodyA.getMaterial(), info.bodyB.getMaterial()).Friction; // float
