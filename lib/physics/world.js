define(function(require) {
    var Vector2 = require('num/num').Vector2,
        ...
        Grid = require('./grid'),
        BodyCollisionInfo = require('./bodycollisioninfo'),
	...
	World.prototype.pushIfMissing = function(array, item) {
		return arrayUtils.pushIfMissing(array, item);
	};

	// TODO: remove this delegation
	World.prototype.removeIfExisting = function(array, item) {
		return arrayUtils.removeIfExisting(array, item);
	};
	...
	World.prototype._collisionPointBody = function(penetratedBody, bodyOfPoint, indexOfPoint) {
		...
		while(indexEdgeStart--) {
			...
			if(percentageToClosestPoint < 0)
				closestPointOnEdge = edgeStart.copy();
			else if(percentageToClosestPoint > 1)
				closestPointOnEdge = edgeEnd.copy();
			else
				closestPointOnEdge = new Vector2(
	...
	World.prototype.debugDraw = function(debugDraw) {
		...
		// draw Bodies
		for(i = 0; i < this.mBodies.length; i++) {
			this.mBodies[i].debugDraw(debugDraw);
		}
		
		// draw CollisionList
		for(i = 0; i < this.mCollisionList.length; i++) {
			this.mCollisionList[i].debugDraw(debugDraw);
		}

		// draw TriggerFields
		for(i = 0; i < this._triggerFields.length; i++) {
