...
Jiquid.System.prototype.update = function() {
	...
	for (var pi = 0, il = this.particles.length; pi < il; pi++) {
		...
		for (var i = 0; i < 3; i++) {
			...
			for (var j = 0; j < 3; j++) {
				var n = this.grid[ga+j];
				phi = pxi * p.py[j];
				n.u += phi*p.velocity.x;
				n.v += phi*p.velocity.y;
			}
		}
	}
	...
	for (var pi = 0, il = this.particles.length; pi < il; pi++) {
		...
		p.position.x += gu;
		p.position.y += gv;
		p.velocity.x += this.materials[0].smoothing*(gu-p.velocity.x);
		p.velocity.y += this.materials[0].smoothing*(gv-p.velocity.y);
		p.gridVelocity.x = gu;
		p.gridVelocity.y = gv;
