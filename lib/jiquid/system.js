...
Jiquid.System = function() {
	...
	this.materials.push(new Jiquid.Material()
		.addDebugGui(this.datGui.addFolder("Mat1"))
		.setParticleMass(1)
		.setRestDensity(2)
		.setStiffness(1)
		.setBulkViscosity (1)
	...
	this.materials.push(new Jiquid.Material()
		.addDebugGui(this.datGui.addFolder("Mat2"))
		.setParticleMass(1)
		.setRestDensity(8)
		.setStiffness(0.0625)
...
Jiquid.System.prototype.handleInteraction = function() {
	...
	if(env.input.state(Jiquid.Mouse.LeftButton) || env.input.state(Jiquid.Mouse.RightButton)) {
		// drag
		if(env.input.state(Jiquid.Mouse.LeftButton)) {
			_.each(this.particles, function(p) {
				if(p.position.sub(this.interactionCurrentPoint).lengthSquared() < 50)
					p.velocity.addSelf(this.interactionCurrentPoint.sub(this.interactionLastPoint)
							//.mulFloat(0.1)
							.mulFloat(0.1 * Math.log((1+p.position.sub(this.interactionCurrentPoint).lengthSquared())))
							);
			}, this);
		} else
			// attract
			if(env.input.state(Jiquid.Mouse.RightButton)) {
			_.each(this.particles, function(p) {
