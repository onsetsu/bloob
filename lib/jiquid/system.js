...
Jiquid.System = function(layer, world) {
	...
	this.boundaries.Min.x = this.particles[0].position.x;
	this.boundaries.Min.y = this.particles[0].position.y;
	this.boundaries.Max.x = this.boundaries.Min.x;
	this.boundaries.Max.y = this.boundaries.Min.y;

	for (var i = 0, il = this.particles.length; i < il; i++) {
		var p = this.particles[i];
		if (p.position.x<this.boundaries.Min.x) {
			this.boundaries.Min.x = p.position.x;
		} else if (p.position.x>this.boundaries.Max.x) {
			this.boundaries.Max.x = p.position.x;
		}
		if (p.position.y<this.boundaries.Min.y) {
			this.boundaries.Min.y = p.position.y;
		} else if (p.position.y>this.boundaries.Max.y) {
			this.boundaries.Max.y = p.position.y;
		}
	}
