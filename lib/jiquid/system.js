...
Jiquid.System.prototype.handleInteraction = function() {
	...
	if(env.input.state(Jiquid.Mouse.LeftButton) || env.input.state(Jiquid.Mouse.RightButton) || env.input.state("up")) {
		// drag
		...
		else if(env.input.state(Jiquid.Mouse.RightButton)) {
			// attract
			_.each(this.particles, function(p) {
