...
Jiquid.System = function(layer, world) {
	...
	minX = this.particles[0].position.x;
	minY = this.particles[0].position.y;
	maxX = minX;
	maxY = minY;
...
Jiquid.System.prototype.update = function() {
	...
	for (var pi = 0, il = this.particles.length; pi < il; pi++) {
		...
		p.cx = Math.floor(p.position.x - minX - 0.5);
		p.cy = Math.floor(p.position.y - minY - 0.5);
		p.gi = p.cx * gsizeY + p.cy;

		var x = p.cx - (p.position.x-minX);
