define(function(require) { 'use strict';
    ...
    var Animation = require('engine/rendering/animation'),
	...
	var Entity = mini.Class.subclass({
		 ...
		 *  components
		 */
		addComponent: function(componentClass, component) {
		...
		removeComponent: function(componentClass, component) {
			if(this.components.has(componentClass)) {
		...
		addDrawCallback: function(callback) {
			this.addComponent(DrawCallback, callback);
		},
		
		clearDrawCallbacks: function() {
			this.clearComponents(DrawCallback);
		},
		
		applyDrawCallbacks: function(renderer) {
			for(let drawCallback of this.getComponents(DrawCallback)) {
				drawCallback.draw(this, renderer);
			}
		},
	...
	Entity.prototype.toJson = function() {
		...
		var drawCallbacks = this.getComponents(DrawCallback);
		if(drawCallbacks.length > 0) {
			json.drawCallbacks = [];
			for(let drawCallback of drawCallbacks) {
