...
.defines(function(
	...
	Layer.WorldLayer = Layer.subclass({
		...
		update: function(timePassed) {
			...
			Layer.prototype.update.call(this, timePassed);
		...
		debugDraw: function(debugDraw) {
			Layer.prototype.debugDraw.call(this, debugDraw);
			...
			var moveCamera = new InteractionHandler()
				...
				.onState(function(timePassed) {
					...
					env.camera.translateBy(
						env.camera.screenToWorldCoordinates(this.screenCurrentCoordinates).sub(
							env.camera.screenToWorldCoordinates(this.screenStartCoordinates)
						)
					);
					...
					env.camera.translateBy(
						env.camera.screenToWorldCoordinates(this.screenStartCoordinates).sub(
							env.camera.screenToWorldCoordinates(this.screenCurrentCoordinates)
						)
					);
				});
	...
	Layer.fromJson = function(json) {
		...
		layer.entities.push(new EntityTest());
		return layer;
	};
