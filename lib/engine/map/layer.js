...
.defines(function(
	...
	var Layer = mini.Class.subclass({
		...
		initDatGui: function() {},
		initMouseInteractionMap: function() {},
		initMouseInteractionEditor: function() {}
	});
	...
	Layer.WorldLayer = Layer.subclass({
		...
		initMouseInteractionMap: function(mouse, camera, datGuiFolder) {
			var tecDragBody = Builder.buildDragBodyInWorld(this.world, camera);
			var tecSpawnCube = Builder.buildSpawnCubeInWorld(this.world, camera);
			var tecSelectBody = Builder.buildSelectBodyInWorld(this.world, camera);
		...
		initMouseInteractionEditor: function(mouse, camera, datGuiFolder) {
				...
				.onPressed(function(timePassed) {
					Jello.BodyFactory.createBluePrint(Jello.SpringBody)
						...
						.shape(new Jello.ClosedShape()
							...
							.finish())
						.pointMasses(1)
						.translate(camera.screenToWorldCoordinates(env.input.mouse))
						.rotate(0)
			...
			var moveBody = new InteractionHandler()
				...
				.onPressed(function(timePassed) {
					...
					var answer = this.world.getClosestPointMass(this.camera.screenToWorldCoordinates(env.input.mouse));
				...
				.onState(function(timePassed) {
					if(typeof this.body !== "undefined") {
						...
						var cursorOffsetInWorld = this.camera.screenToWorldCoordinates(env.input.mouse)
			...
			var movePointMass = new InteractionHandler()
				...
				.onState(function(timePassed) {
					...
					var cursorOffsetInWorld = Jello.VectorTools.rotateVector(
						this.camera.screenToWorldCoordinates(env.input.mouse)
							.sub(this.camera.screenToWorldCoordinates(this.screenCurrentCoordinates)),
	...
	// Enhance Layer.fromJson to include WorldLayer as well.
	Layer.fromJson = function(json) {
