...
.defines(function(
	...
	var Map = mini.Class.subclass({
		initialize: function() {
			this._world = new Jello.World();
			
			this.callbacks = [];
			this.layers = [];
			
			this.layers.push(new Layer.WorldLayer().setWorld(this._world));
		},
		...
		onBuildFinished: function(callback) {
		...
		initDatGui: function(datGui) {
			...
			mapFolder.open();
			mapFolder.add(this.getWorld().materialManager.mMaterialPairs[Jello.Material.Default][Jello.Material.Default], 'Friction').name('Friction').min(-2).max(2).listen().step(0.1);
			mapFolder.add(this.getWorld().materialManager.mMaterialPairs[Jello.Material.Default][Jello.Material.Default], 'Elasticity').name('Elasticity').min(-2).max(2).listen().step(0.1);
