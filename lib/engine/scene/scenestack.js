	...
	var SceneStack = mini.Class.subclass({
		...
		run: function(scene) {
			...
			if(stackSize === 0) {
				...
				/*
				 * TODO: is this necessary as the previous Scene is already stopped?
				 * i.e. the second Loader.load in the method loadAndRun stops the
				 * current Scene explicitly by pause, then stops that Scene again
				 * by calling run
				 */
