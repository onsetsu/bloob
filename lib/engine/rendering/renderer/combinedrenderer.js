...
define([
	...
	"physics/jello",
	"num/matrix3x3"
], function(Configuration, Jello, AffineTransformation) {
	...
	var CombinedRenderer = mini.Class.subclass({
		...
		pushLayer: function(layer) {
            ...
            transform.scaleSelf(Jello.Vector2.One.divVector(env.camera.viewport.extent));

			// move current world camera point to the coordinate system's origin
            transform.translateSelf(env.camera.viewport.point.negative().mulVector(layer.scrollFactor));
