...
define([
	...
	"physics/jello",
	'num/num'
], function(require, Configuration, Jello, num) {
    var AffineTransformation = num.AffineTransformation,
        Vector2 = num.Vector2,
        AABB = require('physics/jello').AABB;
	...
	var CombinedRenderer = mini.Class.subclass({
		...
		setCanvas: function(canvas) {
			...
			// TODO: HACK? http://html5hub.com/state-of-nearest-neighbor-interpolation-in-canvas/
this.context.mozImageSmoothingEnabled = false;
    ...
    this.context.imageSmoothingEnabled = false;
		...
		pushLayer: function(layer) {
            ...
            var invViewportTransformation = env.camera.viewport.getTransformationForLayer(layer).inverted();
            transform.concatenateSelf(invViewportTransformation);
		...
		drawCircle: function(vec, radius, lineColor, opacity, lineWidth) {
			this.drawCount++;

			this.configuration.setStrokeStyle(lineColor);
			this.configuration.setGlobalAlpha(opacity);
		...
		drawImageTriangleOnWorldTriangle: function(body, image, textureMapping) {
			...
			var targetX = 0;
			var targetY = 0;
			var targetWidth = imageData.width;
			var targetHeight = imageData.height;
