...
.defines(function(Renderer) {
	...
	var DebugDraw = Renderer.subclass({
		...
		isVisible: function(aabb) {
			/*
			this.drawText("test", new Vector2(0, env.canvas.canvasHeight/2));
			this.drawText("test", new Vector2(env.canvas.canvasWidth/2, env.canvas.canvasHeight/2));
			this.drawText("test", new Vector2(env.canvas.canvasWidth/2, 0));
			this.drawText("test", new Vector2(0, 0));
			*/
			if(!this.canvasWorldAABB) {
