define([
	...
	var Tool = mini.Class.subclass({
		initialize: function Tool() {
			...
			this._lastPosition = new Vector2(env.input.mouse.x, env.input.mouse.y);
		...
		onDraw: function(callback) { this._drawCallback = callback; },
		...
		update: function() {
			var position = new Vector2(env.input.mouse.x, env.input.mouse.y);
			...
			this._tempEvent = event;
		},

		draw: function() {
		    this._drawCallback.call(this, this._tempEvent);
		    this._tempEvent = undefined;
		}
	});
