define(['require', 'num/num'], function(require, num) {
	...
	var Input = mini.Class.subclass({
		initialize: function(domElementId) {
			 ...
			 */
			this.axes = {
				left: Vector2.Zero.copy(),
				right: Vector2.Zero.copy()
			};
			/**
		...
		reportOnGamepad: function() {
			...
			this.trigger.left = gp.buttons[6].value;
			this.trigger.right = gp.buttons[7].value;
		...
		getAxisWithDeadzone: function(vector, horizontal, vertical) {
			var deadzone = 0.25;
			vector.setXY(horizontal, vertical);
			if(vector.length() < deadzone) {
				vector.set(Vector2.Zero);
			}
			else {
