...
wm.Weltmeister = Game.subclass({	
	...
	initialize: function() {
		...
		env.system = {
			context: env.canvas.getContext('2d')
		};
		...
		for( var key in wm.config.binds ) {
			ig.input.bind( EventedInput.KEY[key], wm.config.binds[key] );
		}
		ig.input.keydownCallback = Utils.Function.bind(this.keydown, this);
		ig.input.keyupCallback = Utils.Function.bind(this.keyup, this);
		ig.input.mousemoveCallback = Utils.Function.bind(this.mousemove, this);
