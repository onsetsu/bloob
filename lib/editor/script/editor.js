...
wm.Weltmeister = Game.subclass({	
	...
	initialize: function() {
		...
		this.resize();
		//env.loop.add(this, this.drawIfNeeded);
	},
	...
	resize: function() {
		...
		*/
		...
		env.camera.width = desiredWidth;
		env.camera.height = desiredHeight;
	...
	loadNew: function() {
		...
		this.editorScene.setMap(emptyMap).run();
	...
	load: function( dialog, name ) {
		var that = this;

		this.fileName = name.replace(/^.*\//,'');
		this.filePath = this.fileNameToFilePath(name);
	...
	loadResponse: function( mapOrData ) {
		var data = mapOrData;
		this.editorScene.setMap(mapOrData).run();
	...
	drawLabels: function( renderer, step ) {
		var drawXAxis = function(text, screenX) {
			renderer.drawText(
				text,
				new Vector2(screenX, 10)
			);
		};
		
		var minXWorld = env.camera.screenToWorldCoordinates(new Vector2(0, 0)).x;
		var maxXWorld = env.camera.screenToWorldCoordinates(new Vector2(env.canvas.canvasWidth, 0)).x;
		...
		var minYWorld = env.camera.screenToWorldCoordinates(new Vector2(0, env.canvas.canvasHeight)).y;
