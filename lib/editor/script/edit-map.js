	...
	debugDraw: function(debugDraw) {
		...
		*/
		...
		if( this.active && wm.config.view.grid ) {
			...
			// Vertical lines.
			var upperY = env.camera.screenToWorldCoordinates(new Vector2(0, env.canvas.canvasHeight)).y;
			var lowerY = env.camera.screenToWorldCoordinates(new Vector2(0, 0)).y;
			var drawVerticalLine = function(worldX) {
				env.renderer.drawPolyline([
			...
			var rightX = env.camera.screenToWorldCoordinates(new Vector2(env.canvas.canvasWidth, 0)).x;
