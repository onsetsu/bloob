	...
	debugDraw: function(debugDraw) {
		...
		*/
		...
		if( this.active && wm.config.view.grid ) {
			...
			for(x = 0; x < maxXWorld; x += wm.config.view.gridStep) {
				drawVerticalLine( x );
			}
			for(x = 0; x > minXWorld; x -= wm.config.view.gridStep) {
				drawVerticalLine( x );
			}
			...
			for(y = 0; y < maxYWorld; y += wm.config.view.gridStep) {
				drawHorizontalLine( y );
			}
			for(y = 0; y > minYWorld; y -= wm.config.view.gridStep) {
				drawHorizontalLine( y );
			}
		}
