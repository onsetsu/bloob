define([
	...
	System.prototype.surfaceTensionImplementation = function() {
		_.each(this.particles, function(p, pIndex) {
			...
			var m = p.material.particleMass;
			var mVelocity = p.velocity.mulFloat(m);
			var mi = p.material.materialIndex;
			
			this.integrator.integrate(p, function(particle, node, phi, gxpy, pxgy) {
				node.m += phi * m;
				node.particleDensity += phi;
				node.velocity.addSelf(mVelocity.mulFloat(phi));
				node.cgx[mi] += gxpy;
				node.cgy[mi] += pxgy;
			});
		}, this);
		...
		this.grid.iterate(function(n) {
			if (n.m > 0.0) {
				n.acceleration.set(Jello.Vector2.Zero);
				n.gx = 0;
				n.gy = 0;
				n.velocity.divFloatSelf(n.m);
				
				var numMaterials = this.materials.length;
				for(var i = 0; i < numMaterials; i++) {
					n.gx += n.cgx[i];
					n.gy += n.cgy[i];
				}
				for(var i = 0; i < numMaterials; i++) {
					n.cgx[i] -= n.gx - n.cgx[i];
					n.cgy[i] -= n.gy - n.cgy[i];
				}
			}
		}, this);
		...
		_.each(this.particles, function(p, pIndex) {
			var fx = 0, fy = 0,
				dudx = 0, dudy = 0, // velocity gradients
				dvdx = 0, dvdy = 0, // velocity gradients
				sx = 0, sy = 0; // surface tension
			...
			this.integrator.integrate(p, function(particle, node, phi, gxpy, pxgy) {
				...
				sx += phi * node.cgx[particle.material.materialIndex];
				sy += phi * node.cgy[particle.material.materialIndex];
			});
		}, this);
