...
.defines(function() {
	...
	Audio.prototype._initAudioGraph = function(datGui) {
		// init audio graph
		try {
	...
	Audio.prototype._initDatGui = function(datGui) {
		var audioFolder = datGui.addFolder("Audio");
		audioFolder.open();
		audioFolder.add(this.trackParent.gain, 'value').name('TrackVolume').min(0.0).max(1.0).listen().step(0.1);
		audioFolder.add(this.soundParent.gain, 'value').name('SoundVolume').min(0.0).max(1.0).listen().step(0.1);
