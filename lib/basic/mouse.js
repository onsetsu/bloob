...
.defines(function(MapBuilder, Input) {
	var Mouse = function(canvas) {
		this.input = new Input(canvas.canvasId);
		this.input.initMouse();
		this.input.bind(Input.KEY.MOUSE1, Bloob.Mouse.LeftButton);
		this.input.bind(Input.KEY.MOUSE2, Bloob.Mouse.RightButton);
		
		this.onLeftClick(new Bloob.InteractionHandler());
		this.onRightClick(new Bloob.InteractionHandler());
	};
