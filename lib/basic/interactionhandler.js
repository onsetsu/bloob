...
.defines(function() {
	var InteractionHandler = mini.Class.subclass({
		...
		fireCallback: function(button) {
			this.callback.call(button);
			if(env.input.pressed(button))
				this.store.onPressed.call(this.store.context);
			if(env.input.state(button))
				this.store.onState.call(this.store.context);
			if(env.input.released(button))
				this.store.onReleased.call(this.store.context);
		}
	});
