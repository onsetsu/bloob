...
.defines(function(ShapeBuilder, Utils, BodyEnhancement) {
	var MapBuilder = {
		...
		"setUpTestMap": function(scene) {
			...
			for(var i = 0; i < numberOfCubes; i++) {
				scene.getWorld().materialManager.addMaterial(i);
			};
			for(var i = 0; i < numberOfCubes; i++) {
				for(var j = 0; j < numberOfCubes; j++) {
					scene.getWorld().materialManager.setMaterialPairCollide(i, j, true);
					scene.getWorld().materialManager.setMaterialPairData(i, j, 
						1,//i/numberOfCubes,//i/(numberOfCubes-1)+0.2,
						i/numberOfCubes//j/(numberOfCubes-1)+0.2
					);
					scene.getWorld().materialManager.setMaterialPairFilterCallback(i, j, new Jello.CollisionCallback());
				};
			};
			...
			new BodyEnhancement(pb).makeSticky(scene.datGuiFolder);
			...
			for(var i = 0; i < 360; i += 20) {
				...
				diamond.onStartContact(function(otherBody, contact) {
					if(typeof otherBody.aName !== "undefined")
						scene.getWorld().queue().removeBody(this);
				});
			 ...
			 *  Rays
			 */
			var ray = new Jello.Ray(scene.getWorld(), new Jello.Vector2(-10, -5), new Jello.Vector2(1, 0.5));
