Bloob.MapBuilder = {
	...
	"setUpTestMap": function(scene) {
		...
		var groundShape = new Jello.ClosedShape()
			.begin();
		Bloob.MapBuilder.generateRoundShape(new Jello.Vector2(0,0), 15.0, 270, 90, 20, groundShape, groundShape.addVertex);
		groundShape
		...
		new Bloob.Entity(pb).makeSticky(scene.datGuiFolder);
		...
		for(var i = 0; i < 360; i += 20) {
			...
			diamond.addOnStartContact(function(otherBody, contact) {
				if(typeof otherBody.aName !== "undefined")
					scene.world.queue().removeBody(this);
			});
		 ...
		 *  Rays
		 */
		var ray = new Jello.Ray(scene.world, new Jello.Vector2(-10, -5), new Jello.Vector2(1, 0.5));
		 ...
		 *  TriggerFields:
		 */
		var trigger = new Jello.TriggerField(
			scene.world, new Jello.AABB(
				new Jello.Vector2(-40, 10),
				new Jello.Vector2(-30, 25)
			)
		).onOverlapBody(function(body) {
			Scarlet.log("has Body");
		}).onContainBody(function(body) {
			Scarlet.log("contains");
		});
