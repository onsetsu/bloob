...
.defines(function(
	...
	var MapBuilder = {
		...
		"setUpTestMap": function(layer, world) {
			...
			// Entity
			var blockEntity = new Entity(0, 10, { foo: 42, bar: 17 }).addToLayer(layer);
			...
			for(var i = 0; i < 4; i++) {
				for(var j = 0; j < 4; j++) {
					...
					// create Entity
					var jellyCrate = new Entity(0, 0, { foo: 42, bar: 17 }).addToLayer(layer);
			...
			// Entity
			var jumpingCrate = new Entity(0, 10, { foo: 42, bar: 17 }).addToLayer(layer);
			...
			for (i = 0; i <= 2; i++) {
				for (j = 0; j <= 3; j++) {
					// Entity
					var fireflyEntity = new Entity(0, 10, { foo: 42, bar: 17 }).addToLayer(layer);
