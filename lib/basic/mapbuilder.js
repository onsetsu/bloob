...
.defines(function(
	...
	var MapBuilder = {
		...
		"setUpMainLayer": function(layer, world) {
			 ...
			 */
			// Entity
			var blockEntity = new Entity("controllableBlob", { foo: 42, bar: 17 }).addToLayer(layer);
			...
			// Behaviour
			blockEntity.setTrait(new Trait("showcase/controllableblob", pb));
			...
			// Entity
			var blockEntity = new Entity("testBlock", { foo: 42, bar: 17 }).addToLayer(layer);
			...
			var stateAnimationShowCase = new Entity("stateAnimation", { foo: 42, bar: 17 }).addToLayer(layer);

			// Body
			stateAnimationShowCase.setBody(Jello.BodyFactory.createBluePrint(Jello.PressureBody)
				.world(world)
			...
			// Entity
			var movingGround = new Entity("movingGround", { foo: 42, bar: 17 }).addToLayer(layer);
			
			// Behaviour
			movingGround.setTrait(new Trait("showcase/movingground"));

			// Body
			movingGround.setBody(Jello.BodyFactory.createBluePrint(Jello.PressureBody)
				.world(world)
				.shape(ShapeBuilder.Shapes.Cube)
				.pointMasses(0)
			...
			for(var i = 0; i < 4; i++) {
				for(var j = 0; j < 4; j++) {
					...
					// create Entity
					var jellyCrate = new Entity("jellyCrate", { foo: 42, bar: 17 }).addToLayer(layer);
			...
			// Entity
			var jumpingCrate = new Entity("jumpingCrate", { foo: 42, bar: 17 }).addToLayer(layer);
			...
			// Entity
			var stickyBlob = new Entity("stickyBlob", { foo: 42, bar: 17 }).addToLayer(layer);
			...
			// Behaviour
			stickyBlob.setTrait(new Trait("showcase/stickiness", pb));
			...
			for (i = 0; i <= 2; i++) {
				for (j = 0; j <= 3; j++) {
					// Entity
					var fireflyEntity = new Entity("firefly", { foo: 42, bar: 17 }).addToLayer(layer);
					
					// Behaviour
					fireflyEntity.setTrait(new Trait("showcase/firefly", fireflyEntity));
		...
		"setUpMenuLayer": function(layer, world) {
			...
			// Entity
			var guiElement = new Entity("guiElement", { foo: 42, bar: 17 }).addToLayer(layer);

			// Behaviour
			guiElement.setTrait(new Trait("showcase/basicguielement"));

			// Body
			guiElement.setBody(Jello.BodyFactory.createBluePrint(Jello.Body)
				.world(world)
				.shape(ShapeBuilder.Shapes.Cube)
				.pointMasses(0)
		...
		"setUpLiquidLayer": function(layer, world) {
			...
			// Sample loft
			var loft = Jello.BodyFactory.createBluePrint(Jello.Body)
				.world(world)
				.shape(groundShape)
				.pointMasses(Utils.Array.fill(0, groundShape.getVertices().length))
