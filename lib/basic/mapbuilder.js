	...
	var MapBuilder = {
		...
		"setUpMainLayer": function(layer, world) {
			...
			var path = new GeoMath.Path()
				.add(new GeoMath.Path.Segment(new Vector2(100, 0)))
			...
			Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(GeoMath.Path.Rectangle(Vector2.Zero.copy(), Vector2.One.copy().mulFloat(4))
					.flatten(0.5)
					.toClosedShape()
				)
				.pointMasses(0)
			...
			Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(GeoMath.Path.Circle(Vector2.Zero.copy(), 4)
					.flatten(2)
					.toClosedShape()
				)
				.pointMasses(0)
