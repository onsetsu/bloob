	...
	var MapBuilder = {
		...
		"setUpMainLayer": function(layer, world) {
			...
			var pb = Jello.BodyFactory.createBluePrint()
				...
				.pointMasses(1.0)
				.translate(new Jello.Vector2(115, 15))
				.rotate(0)
			...
			// ray functionality
			//var hitTestRayCast = new Jello.Ray(layer.world, Jello.Vector2.Zero.copy(), Jello.Vector2.One.copy());
			//var hitTestCircleCast = new Jello.Query.CircleCast(layer.world, Jello.Vector2.Zero.copy(), 1);
			var hitTestRay = new Jello.Query.Ray(Jello.Vector2.Zero.copy(), Jello.Vector2.One.copy());
			var hitTestCircle = new Jello.Query.Circle(Jello.Vector2.Zero.copy(), 1);
			rayTool.onMouseDown(function(event) {
				//layer.world.addRay(hitTestRayCast);
				//layer.world.addRay(hitTestCircleCast);
				//hitTestRayCast.origin.set(event.getPositionInLayer(layer));
				//hitTestCircleCast.origin.set(event.getPositionInLayer(layer));
				hitTestRay.origin.set(event.getPositionInLayer(layer));
				hitTestCircle.origin.set(event.getPositionInLayer(layer));
				this.__isDown__ = true;
			});
			rayTool.onMouseUp(function(event) {
				//layer.world.removeRay(hitTestRayCast);
				//layer.world.removeRay(hitTestCircleCast);
				this.__isDown__ = false;
			});
			rayTool.onMouseDrag(function(event) {
				//hitTestRayCast.direction.set(event.getDownDeltaInLayer(layer));
				//hitTestCircleCast.radius = event.getDownDistanceInLayer(layer);
				hitTestRay.direction.set(event.getDownDeltaInLayer(layer));
				hitTestCircle.radius = event.getDownDistanceInLayer(layer);
			});
			rayTool.onDraw(function() {
                env.renderer.pushLayer(layer);
                layer.world.debugDrawRays(env.renderer);
                if(this.__isDown__) {
