...
define(function(require) {
	var num = require('num'),
		ShapeBuilder = require('basic/shapebuilder'),
		Utils = require('basic/utils'),
		Input = require('engine/input/input'),
		Tool = require('engine/input/tool'),
		Entity = require('engine/map/entity'),
		Component = require('engine/map/component'),
		System = require('engine/map/system'),
		...
		Image = require('engine/rendering/image'),
		AnimationSheet = require('engine/rendering/animationsheet'),
		DrawCallback = require('engine/rendering/drawcallback'),
		Jello = require('jello'),
		Loader = require('assets/loader'),
		wmModalDialog = require('editor/script/modal-dialogs');
	...
	var MapBuilder = {
		...
		"setUpMainLayer": function(layer, world) {
			...
			// Body
			var blobBlueprint = Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(ShapeBuilder.Shapes.Ball)
				.pointMasses(1.0)
				.translate(Vector2.Zero.copy())//new Vector2(115, 15))
				.rotate(0)
				...
				.shapeSpringDamp(1.0)
				.gasPressure(100.0);
			var pb = blobBlueprint.build();
			pb.isPlayer = true;
			blockEntity.setBody(pb);
			var controlledBlob = blockEntity;
			...
			(function() {
			    return env.input.state("D");
			}).trigger(function() {
			    console.log.bind(console, "Trigger: Pressed D")();
			});
			...
			(function() {
			    ...
			    this.disable();
			    console.log.bind(console, "Trigger: Pressed S")();
			});
			...
			// ray functionality
			var hitTestRay = new Jello.Query.Ray(Vector2.Zero.copy(), Vector2.One.copy());
			var hitTestCircle = new Jello.Query.Circle(Vector2.Zero.copy(), 1);
			rayTool.onMouseDown(function(event) {
				hitTestRay.origin.set(event.getPositionInLayer(layer));
				hitTestCircle.origin.set(event.getPositionInLayer(layer));
			});
			rayTool.onMouseDrag(function(event) {
				hitTestRay.direction.set(event.getDownDeltaInLayer(layer));
				hitTestCircle.radius = event.getDownDistanceInLayer(layer);
			});
			rayTool.onDraw(function(event) {
                if(env.input.state(Tool._mouseButton)) {
                    env.renderer.pushLayer(layer);
                    layer.world.raycast(hitTestRay).debugDraw(env.renderer);
                    layer.world.circlecast(hitTestCircle).debugDraw(env.renderer);
                    env.renderer.popLayer(layer);
                }
			});
			...
			(function() { 'use strict';
				...
				class CountSystem extends System {
					update() {
						var trackedEntities = this.getEntitiesMatching(Tracked);

						if(Bloob.debug) {
							Bloob.debug.showNumber('tracked Entities', trackedEntities.length);
						}
					}
				}

				layer.addSystem(new CountSystem());

				(5).times(function (i) {
					new Entity('ecsTest' + i)
						.addToLayer(layer)
						.addComponent(new Tracked());
				});
			})();
			...
			(function() { 'use strict';
				class GamePadSystem extends System {
					constructor() {
						...
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_2, 'B');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_3, 'X');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_4, 'Y');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_5, 'LEFT_SHOULDER_BUTTON');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_6, 'RIGHT_SHOULDER_BUTTON');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_7, 'LEFT_SHOULDER_TRIGGER');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_8, 'RIGHT_SHOULDER_TRIGGER');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_9, 'BACK');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_10, 'START');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_11, 'LEFT_AXIS_BUTTON');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_12, 'RIGHT_AXIS_BUTTON');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_13, 'UP');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_14, 'DOWN');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_15, 'LEFT');
						env.input.bind(Input.GAMEPAD_KEY.BUTTON_16, 'RIGHT');
					}

					update() {
						var controlledEntities = this.getEntitiesMatching(GamepadControlled);

						controlledEntities.forEach(entity => {
							let body = entity.getBody(),
								currentVelocity = body.getDerivedVelocity(),
								desiredVelocity = env.input.axes.left
									.mirrorHorizontal()
									.mulFloat(20),
								velocityChange = desiredVelocity.sub(currentVelocity),
								forceToApply = velocityChange
									//.mulFloat(body.getMass())
									.mulFloat(0.9)
									.divFloat(1/60);
						...
						if(env.input.pressed('A')) {
							if(env.camera.bodyToTrack === controlledEntities[0].getBody()) {
								env.camera.track(controlledBlob.getBody(), layer);
							} else {
								env.camera.track(controlledEntities[0].getBody(), layer);
							}
						}
					}
				}

				layer.addSystem(new GamePadSystem());

				// Entity
				var proto = new Entity("PROTO").addToLayer(layer);

				// Components
				class GamepadControlled extends Component {}
				proto.addComponent(new GamepadControlled());

				// Body
				var controllableBlobBody = blobBlueprint
					.translate(new Vector2(30, 5))
					.build();
				controllableBlobBody.setVelocityDamping(0.9);
				proto.setBody(controllableBlobBody);

				// make blob fly
				controllableBlobBody.clearExternalForces();

				env.camera.track(controllableBlobBody, layer);
			})();
			...
			// Entity
			blockEntity = new Entity("testBlock").addToLayer(layer);
			...
			blockEntity.setBody(Jello.BodyFactory.createBluePrint()
				...
				.shape(new Jello.ClosedShape().begin()
					.addVertex(new Vector2(-1.0, -1.0)) // 0
					.addVertex(new Vector2(-1.0, -0.5))
					.addVertex(new Vector2(-1.0,  0.0))
					.addVertex(new Vector2(-1.0,  0.5))
					.addVertex(new Vector2(-1.0,  1.0)) // 4
					.addVertex(new Vector2(-0.5,  1.0))
					.addVertex(new Vector2( 0.0,  1.0))
					.addVertex(new Vector2( 0.5,  1.0))
					.addVertex(new Vector2( 1.0,  1.0)) // 8
					.addVertex(new Vector2( 1.0,  0.5))
					.addVertex(new Vector2( 1.0,  0.0))
					.addVertex(new Vector2( 1.0, -0.5))
					.addVertex(new Vector2( 1.0, -1.0)) // 12
					.addVertex(new Vector2( 0.5, -1.0))
					.addVertex(new Vector2( 0.0, -1.0))
					.addVertex(new Vector2(-0.5, -1.0))
					.finish())
				.pointMasses(1.0)
				.translate(new Vector2(-75, 25))
				.rotate(0)
				.scale(Vector2.One.copy().mulFloat(4.0))
				.isKinematic(false)
				.edgeSpringK(300.0)
				.edgeSpringDamp(20.0)
				.shapeSpringK(500.0)
			...
			blockEntity.setTexture(new Texture("sample.png")
				.from([new Vector2(0, 32), new Vector2(0, 0), new Vector2(96, 0)]).to([3,4,5])
				.from([new Vector2(0, 64), new Vector2(0, 32), new Vector2(96, 0)]).to([2,3,5])
				.from([new Vector2(0, 64), new Vector2(96, 0), new Vector2(192, 0)]).to([2,5,6])
				.from([new Vector2(0, 96), new Vector2(0, 64), new Vector2(192, 0)]).to([1,2,6])
				.from([new Vector2(0, 96), new Vector2(192, 0), new Vector2(288, 0)]).to([1,6,7])
				.from([new Vector2(0, 128), new Vector2(0, 96), new Vector2(288, 0)]).to([0,1,7])
				.from([new Vector2(0, 128), new Vector2(288, 0), new Vector2(384, 0)]).to([0,7,8])
				.from([new Vector2(96, 128), new Vector2(0, 128), new Vector2(384, 0)]).to([15,0,8])
				.from([new Vector2(96, 128), new Vector2(384, 0), new Vector2(384, 32)]).to([15,8,9])
				.from([new Vector2(192, 128), new Vector2(96, 128), new Vector2(384, 32)]).to([14,15,9])
				.from([new Vector2(192, 128), new Vector2(384, 32), new Vector2(384, 64)]).to([14,9,10])
				.from([new Vector2(288, 128), new Vector2(192, 128), new Vector2(384, 64)]).to([13,14,10])
				.from([new Vector2(288, 128), new Vector2(384, 64), new Vector2(384, 96)]).to([13,10,11])
				.from([new Vector2(384, 128), new Vector2(288, 128), new Vector2(384, 96)]).to([12,13,11])
			);
			...
			smartlyTexturedGround.setTexture(new Texture("sample.png")
				.from([new Vector2(0, 32), new Vector2(0, 0), new Vector2(96, 0)]).to([0,1,2])
				.from([new Vector2(0, 64), new Vector2(0, 32), new Vector2(96, 0)]).to([2,3,0])
			);
			
			var path = new Path()
				.add(new Path.Segment(new Vector2(100, 0)))
				.lineBy(new Vector2(10, 0))
				.cubicCurveBy(new Vector2(10, 0), new Vector2(10, -10), new Vector2(0, -10))
				.quadraticCurveBy(new Vector2(-10, 0), new Vector2(-10, 10))
				.close(true);
			...
			for(i = 0; i < 4; i++) {
				for(j = 0; j < 2; j++) {
					middle = new Vector2(2*18*i, -15 + 36.5*j);
					groundBody = MapBuilder.buildGroundRectangle(
						world,
			...
			middle = new Vector2(-2-10, -25 + 36.5+10);
			groundBody = MapBuilder.buildGroundRectangle(
				world,
				middle.sub(groundShapeScale),
				middle.add(groundShapeScale),
			...
			texturedGround.setBody(Jello.BodyFactory.createBluePrint()
				...
				.shape(ShapeBuilder.Shapes.Cube)
				.pointMasses(0)
				.translate(new Vector2(-52, 0))
				.rotate(0)
				.scale(new Vector2(15, 4))
				.isKinematic(false)
				.edgeSpringK(300.0)
				.edgeSpringDamp(20.0)
				.shapeSpringK(500.0)
			...
			for(i = 0; i < 360; i += anglePerSegment) {
				gearShape.addVertex(new Vector2(
					Math.cos(-i * (Math.PI / 180)),
					Math.sin(-i * (Math.PI / 180))
			...
			var groundShape = new Jello.ClosedShape()
				.begin();
			MapBuilder.generateRoundShape(new Vector2(0,0), 15.0, 270, 90, 20, groundShape, groundShape.addVertex);
			groundShape
			 ...
			 *  Jelly Ground:
			 */
			groundShape = new Jello.ClosedShape()
				.begin();
				// bottom Row
				for(i = 1; i >= -1; i-= 0.25)
			...
			var mass = 1 ;
			groundBody = Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(groundShape)
				.pointMasses([
					0,0,0,0,0,0,0,0,0,
			...
			groundBody = Jello.BodyFactory.createBluePrint()
				...
				.translate(new Vector2(-90, -16)) // translate
				.rotate(0) // rotate
				.scale(new Vector2(18, 3)) // scale
				.isKinematic(false)
				.edgeSpringK(150.0) // float ,
			...
			jellyGroundEntity = new Entity("ground").addToLayer(layer);
			jellyGroundEntity.setBody(groundBody);
			jellyGroundEntity.setTrait(new Trait("showcase/movingground", jellyGroundEntity));
			 ...
			 */
			blockEntity = new Entity("boundaryTest").addToLayer(layer);
			blockEntity.setBody(Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(new Jello.ClosedShape()
					.begin()
					.addVertex(new Vector2(-1, 0))
					.addVertex(new Vector2(-0.75, 0.75))
					.addVertex(new Vector2( 0, 1))
					.addVertex(new Vector2( 0.75, 0.75))
					.addVertex(new Vector2( 1, 0))
					.addVertex(new Vector2( 0.75,-0.75))
					.addVertex(new Vector2( 0,-1))
			...
			for(i = 0; i < 5; i++) {
				blockEntity = new Entity("cube").addToLayer(layer);
				blockEntity.setBody(Jello.BodyFactory.createBluePrint()
					.world(world)
					.shape(new Jello.ClosedShape()
						.begin()
						...
						.addVertex(new Vector2( 0.75,-0.75))
						.addVertex(new Vector2(-0.75,-0.75))
						.addVertex(new Vector2(i,0))
						.finish()
					)
					.boundaryCount(4)
					.pointMasses(1.0)
					.translate(new Vector2(5+5*i, 15))
					.rotate(0)
					.scale(Vector2.One.copy().mulFloat(1.0))
					.isKinematic(false)
					.edgeSpringK(300.0)
					.edgeSpringDamp(20.0)
					.shapeSpringK(150.0)
			...
			blockEntity.setBody(Jello.BodyFactory.createBluePrint()
				...
				.shape(new Jello.ClosedShape()
					.begin()
					.addVertex(new Vector2(0, 1)) // upper left
					.addVertex(new Vector2(1, 1))
					.addVertex(new Vector2(1, 0))
					.addVertex(new Vector2(0, 0))
					.addVertex(new Vector2( 0.5, 2.0))
					.addVertex(new Vector2( 2.0, 0.5))
					.addVertex(new Vector2( 0.5,-1.0))
					.addVertex(new Vector2(-1.0, 0.5))
					.finish()
				)
			...
			Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(Path.Rectangle(Vector2.Zero.copy(), Vector2.One.copy().mulFloat(4))
					.flatten(0.5)
					.toClosedShape()
				)
				.pointMasses(0)
			...
			Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(Path.Circle(Vector2.Zero.copy(), 4)
					.flatten(2)
					.toClosedShape()
				)
				.pointMasses(0)
				.translate(center.add(new Vector2(0, 5)))
				.rotate(0)
				.scale(Vector2.One.copy())
				.isKinematic(false)
				.build();
			...
			for(i = 0; i < numberOfCubes; i++) {
				// materialized ground
				body = Jello.BodyFactory.createBluePrint()
					.world(world)
					.shape(shape)
					.pointMasses(Utils.Array.fill(0, //Number.POSITIVE_INFINITY
						shape.getVertices().length))
					.translate(new Vector2(3*numberOfCubes*i + origin.x, -10+origin.y))
					.rotate(0)
					.scale(new Vector2(3*numberOfCubes/2, 1))
					.isKinematic(false)
				...
				for(j = 0; j < numberOfCubes; j++) {
					...
					body = Jello.BodyFactory.createBluePrint()
						...
						.shape(particleShape)
						.pointMasses(1)
						.translate(new Vector2(3*numberOfCubes*i + 3*j - 4.25 + origin.x, 5+origin.y))
						.rotate(0)
						.scale(Vector2.One.copy().mulFloat(1.0))
						.isKinematic(false)
			...
			bodyBluePrintForCollisionType
				.translate(origin.add(new Vector2(0, 5)))
			...
			bodyBluePrintForCollisionType
				.translate(origin.add(new Vector2(0, 10)))
				.collisionType(new Jello.CollisionType()
					.setCollisionOn(specialCollisionId))
				.build();

			bodyBluePrintForCollisionType
				.translate(origin.add(new Vector2(0, 15)))
				.collisionType(new Jello.CollisionType()
					.setCollisionOn(specialCollisionId))
				.build();

			bodyBluePrintForCollisionType
				.translate(origin.add(new Vector2(0, 20)))
				.collisionType(new Jello.CollisionType()
					.setCollisionOn(specialCollisionId))
				.build();
			...
			var body = Jello.BodyFactory.createBluePrint()
				...
				.translate(new Vector2(-2, -9).add(origin)) // translate
				.rotate(0) // rotate
				.scale(Vector2.One.copy().mulFloat(1.0)) // scale
				.isKinematic(false)
			...
			var body2 = Jello.BodyFactory.createBluePrint()
				...
				.shape(shape)
				.pointMasses(1) // mass per point
				.translate(new Vector2(0, -9).add(origin)) // translate
				.rotate(0) // rotate
				.scale(Vector2.One.copy().mulFloat(1.0)) // scale
				.isKinematic(false)
				.edgeSpringK(300.0) // float edgeSpringK,
			...
			var lastParticle = particleBluePrint
				.translate(new Vector2(30, 115))
				.build();
			pb = pressureBallBluePrint
				.translate(new Vector2(40, 10))
				.scale(Vector2.One.copy().mulFloat(2.0))
				.gasPressure(100.0)
				.build();
			...
			var stick = Jello.BodyFactory.createBluePrint()
				...
				.shape(stickShape)
				.pointMasses(1) // mass per point
				.translate(new Vector2(0+30, 12)) // translate
				.rotate(0.0) // rotate
				.scale(Vector2.One.copy().mulFloat(2)) // scale
				.isKinematic(false)
			...
			for (i = 1; i <= 5; i++) {
				stick2 = Jello.BodyFactory.createBluePrint()
					.world(world)
					.shape(stickShape)
					.pointMasses(1) // mass per point
					.translate(new Vector2(2*i+30, 12)) // translate
					.rotate(0.0) // rotate
					.scale(Vector2.One.copy().mulFloat(2)) // scale
					.isKinematic(false)
					.edgeSpringK(150.0) // float edgeSpringK,
			...
			stick.getPointMass(1).Mass = 0;
			
			/*
			 *  Chain of Sticks 2
			 */
			stickShape = new Jello.ClosedShape()
				.begin()
				.addVertex(new Vector2(0.0, 0.0))
				.addVertex(new Vector2(0.0, 0.0))
			...
			center = new Vector2(-30, -25);
			stick = Jello.BodyFactory.createBluePrint()
				.world(world)
				.shape(stickShape)
				.pointMasses(1) // mass per point
			...
			for (i = 1; i <= 10; i++) {
				stick2 = Jello.BodyFactory.createBluePrint()
					...
					.shape(stickShape)
					.pointMasses(1) // mass per point
					.translate(new Vector2(3*i, 0).add(center)) // translate
					.rotate(0.0) // rotate
					.scale(Vector2.One.copy().mulFloat(3)) // scale
					.isKinematic(false)
					.edgeSpringK(450.0) // float edgeSpringK,
					.edgeSpringDamp(5.0) // float edgeSpringDamp,
					.shapeSpringK(300.0) // float shapeSpringK,
			...
			stick.getPointMass(1).Mass = 0;
			stick.getPointMass(2).Mass = 0;

			/*
			 *  TriggerFields:
			 */
			var triggerAABBMin = new Vector2(-40, 10);
		...
		"setUpTitleLayer": function(layer, world) {
			...
			weight.setBody(Jello.BodyFactory.createBluePrint()
				...
				.shape(new Jello.ClosedShape()
					.begin()
					.addVertex(new Vector2(-1.0, -1.0))
					.addVertex(new Vector2(-0.7,  1.0))
					.addVertex(new Vector2( 0.7,  1.0))
					.addVertex(new Vector2( 1.0, -1.0))
					.finish())
				.pointMasses(1)
				.translate(new Vector2(0, 22))
				.rotate(0)
			...
			jelloBall.setBody(Jello.BodyFactory.createBluePrint()
				...
				.shape(ShapeBuilder.Shapes.Ball)
				.pointMasses(1.0)
				.translate(new Vector2(0, -10))
				.rotate(0)
				.scale(Vector2.One.mulFloat(3.5))
				.isKinematic(false)
				.edgeSpringK(300.0)
				.edgeSpringDamp(20.0)
				.shapeSpringK(150.0)
			...
			ground.setBody(Jello.BodyFactory.createBluePrint()
				...
				.shape(ShapeBuilder.Shapes.Cube)
				.pointMasses(0)
				.translate(new Vector2(0,-15))
				.rotate(0)
				.scale(new Vector2(10, 2))
				.isKinematic(false)
				.build()
			);
		...
		"setUpMenuLayer": function(layer, world) {			
			...
			jelloBall.setBody(Jello.BodyFactory.createBluePrint()
				...
				.translate(new Vector2(0, -10))
				.rotate(0)
				.scale(Vector2.One.mulFloat(300.5))
				.isKinematic(false)
				.edgeSpringK(300.0)
				.edgeSpringDamp(20.0)
				.shapeSpringK(150.0)
