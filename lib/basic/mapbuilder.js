Bloob.MapBuilder = {
	...
	"setUpTestMap": function(game) {
		...
		var groundBody = Bloob.BodyFactory.createBluePrint(Body)
			.world(game.world)
			.shape(groundShape)
			.pointMasses(Utils.fillArray(0, groundShape.getVertices().length))
		...
		for(var i = 0; i < 10; i++)
			fuzzyEllipseGround(
				game.world,
				new Vector2(Math.floor(Math.random() * -100), Math.floor(Math.random() * -100)),
				0,
		...
		for(var i = 0; i < 4; i++) {
			for(var j = 0; j < 4; j++) {
				var body = jellyCubeBluePrint
					.translate(new Vector2(25+2*i, 2*j))
					.build();
				body.aName = "Jelly";
			}
		};
