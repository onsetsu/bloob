define(function(require) {
	var LoadAssembler = require('assets/loadassembler');
	...
	var Loader = {
		...
		load: function(onFinished, onFinishedContext) {
			...
			var currentResources = [],
			    resourceIndex;
			for(resourceIndex in Loader.resources) {
			...
			for(resourceIndex in currentResources) {
				if(!currentResources.hasOwnProperty(resourceIndex)) continue;
				
				currentResources[resourceIndex].load(singleLoadingFinished);
			}
		}
	};
