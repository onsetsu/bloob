...
.defines(function() {
	EditorScene = function(game, loop) {
		// call parent
	...
	EditorScene.prototype = new chain();
	// enable static method inheritance
	EditorScene.__proto__ = Bloob.MapScene;
	EditorScene.prototype.constructor = chain;
	EditorScene.prototype.parent = Bloob.MapScene.prototype;

	EditorScene.prototype.update = function(timePassed) {
		// rendering
	...
	EditorScene.prototype.initMouseInteraction = function() {
		var that = this;
	...
	EditorScene.prototype.testChangeMap = function() {
		console.log(arguments);
		this.game.datGui.removeFolder(this.sceneID);
		this.stop();

		var mapScene = new Bloob.MapScene(this.game, this.loop);
		Bloob.Map.fromJson(this.map.toJson(), mapScene.map);
		mapScene.map.spawnPlayerAt(mapScene, mapScene.datGuiFolder);
		mapScene.run();
	};

	Bloob.EditorScene = EditorScene;
