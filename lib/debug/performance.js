...
.defines(function(domReady, Menu, DebugPanel, ff, Utils){ "use strict";
...
ff.DebugGraphPanel = DebugPanel.subclass({
	clocks: {},
	marks: [],
	textY: 0,
	height: 128,
	...
	initialize: function( name, label ) {
		...
		this.mark16ms = Utils.round(this.height - (this.height/this.ms) * 16);
		this.mark33ms = Utils.round(this.height - (this.height/this.ms) * 33);
		...
		var that = this;
		ff.mark = function() { that.mark.apply(that, arguments); };
		ff.graph = this;
	},
	...
	addGraphMark: function( name, height ) {
		...
		span.text(name);
		span.css("top", Utils.round(height) + 'px');
...
domReady(function() {
	ff.debug.addPanel({
		type: ff.DebugGraphPanel,
		name: 'graph',
		label: 'Performance'
	});
});

return ff.DebugGraphPanel;
