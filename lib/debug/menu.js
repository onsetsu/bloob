...
Bloob.Debug = mini.Class.subclass({
	...
	afterRun: function() {
		...
		if( env.scene && env.scene.getMap ) {
			// calculate number of entities in all layers
			var numberOfEntities = _.chain(env.scene.getMap().getLayers())
			...
			var numberOfBodies = _.chain(env.scene.getMap().getLayers())
			...
			var numberOfParticles = _.chain(env.scene.getMap().getLayers())
