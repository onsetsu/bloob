...
.defines(function(domReady, Menu, DebugPanel, ff, MapScene, Utils){ "use strict";

/**
 *  ---------------------------- LAYERS PANEL ----------------------------
 */
var mapSceneSetMap = MapScene.prototype.setMap;
MapScene.prototype.setMap = function(map) {
	var returnValue = mapSceneSetMap.apply(this, arguments);
	ff.debug.panels.layers.load(map);
	return returnValue;
};

ff.DebugMapsPanel = ff.DebugPanel.subclass({
	mapScreens: [],
	
	initialize: function( name, label ) {
		ff.DebugPanel.prototype.initialize.apply(this, arguments);
		this.layers = [];
		this.load();
	},
	...
	load: function( map ) {
		...
		for( var m = 0; m < this.layers.length; m++ ) {
			...
			var subPanel = new ff.DebugPanel( m, 'Layer '+m );
			
			var head = new $('<strong />');
			head.text("Layer: " + m);
			subPanel.container.append( head );
			
			subPanel.addOption( new ff.DebugOption('Enabled', layer, 'enabled') );
